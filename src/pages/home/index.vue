<template>
  <div class="demo-list-wrapper">
    ...
    <h1 class="logo"></h1>
    <div class="version">Syberh 1.4.0</div>
    <p>syberos h5开发框架</p>
    <div :class="['demo-list-box',{'unfold':foldStatus[item]}]" v-for="item in sorts" :key="item">
      ...
      <ul>
        <template v-for="cpt in packages">
          <li v-if="cpt.sort == item" :key="cpt.name">
            <a :href="'./#/'+cpt.name">
              {{cpt.name}}
              <span>{{cpt.chnName || ""}}</span>
              <nut-rate class="cpt-rec" v-if="cpt.star" :total="5" :value="cpt.star" :size="8" :spacing="3" :readOnly="true"></nut-rate>            </a>
          </li>
        </template>
      </ul>
    </div>
    <div id="demo-footer">元心科技 · syberh · v{{version}}</div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.oriSorts = sorts;
    this.sorts = new Array(sorts.length).fill(0).map((item, idx) => idx)
